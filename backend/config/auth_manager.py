...
# YouTube API scopes - MUST match what was used in original auth
SCOPES = [
...
def auto_refresh_worker():
    ...
    Runs every 30 minutes and refreshes tokens that expire within 2 hours.
    """
...
# ==============================================================================
# YouTube Upload Functions (from old auth_manager)
# ==============================================================================
...
def get_video_id_from_url(youtube_url: str):
...
def upload_thumbnail(video_id: str, channel_name: str, thumbnail_path: str) -> tuple:
...
def get_youtube_channel_info(channel_name: str):
    ...
    try:
        ...
        if response.get('items'):
            ...
            profile_pic = thumbnails.get('medium', {}).get('url') or thumbnails.get('default', {}).get('url')
...
def generate_youtube_metadata(script: dict) -> dict:
...
def get_channel_credentials(channel_name: str):
    """Compatibility wrapper"""
    return get_valid_credentials(channel_name)
